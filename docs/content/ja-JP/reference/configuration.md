...
Live2D は RitsuLib の `settings` キーに `settings.json` を保存します。現在の `SchemaVersion` は `6` です。手動編集よりゲーム内 UI を使用してください。
...
各モデルは安定 ID、表示名、管理 model3 パス、元パス記録、内容ハッシュ、インポート時刻、表示順、上書き、検出アクション、
ホットキーを保存します。アクション種別は現在 `0` が Motion、`1` が Expression です。
