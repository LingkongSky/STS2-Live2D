...
推送与项目版本一致的 `v*` 标签后，`publish-nuget.yml` 会从固定版本的私有
`STS2-API-Signatures` 仓库构建并验证包，再通过 NuGet Trusted Publishing 的 OIDC 身份发布。
NuGet.org 策略须绑定该工作流和 `nuget` environment；跨仓库读取签名需要配置只读的
`STS2_SIGNATURES_TOKEN` repository secret。
